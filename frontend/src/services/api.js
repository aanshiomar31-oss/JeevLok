...
export async function parseSymptomsNLP(text) {
  const response = await apiClient.post("/nlp/parse-symptoms", { text });
  return response.data;
}

/** Normalize a colloquial symptom into its standardized medical concept and ICD-10 code */
export async function normalizeSymptom(query) {
  const response = await apiClient.post("/nlp/normalize", { query });
  return response.data;
}

/** Generate an explainable LLM clinical summary after triage prediction */
export async function generateClinicalSummary(patientData, triageResult) {
...
/** Query the RAG-grounded Clinical Copilot conversational assistant */
export async function queryCopilotChat({ message, sessionId = "default_session", patientContext = null, history = null }) {
...
/** Transcribe audio dictation or send browser-transcribed text to populate triage features */
export async function transcribeVoice({ audioBlob = null, transcriptText = null } = {}) {
...
export async function evaluateCounterfactual(basePatient, adjustedVitals) {
  const response = await apiClient.post("/explain/counterfactual", {
    base_patient: basePatient,
    adjusted_vitals: adjustedVitals,
  });
  return response.data;
}

/** Generate plain-language SHAP narrative and impact breakdown */
export async function generateSHAPNarrative(patientData, triageResult) {
  const response = await apiClient.post("/explain/narrative", {
    patient_data: patientData,
    triage_result: triageResult,
  });
  return response.data;
